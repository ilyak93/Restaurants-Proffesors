...
public class CartelDeNachosImpl implements CartelDeNachos {
    ...
    /*
    Each call of getRecommendation implemets an efficient graph search algorithm using
    the graphSerachClass which saves for each Professor if we allready checked him. Each
    Proffessor will be cheked only once, so the complexity of the run is O(Pn)
    where Pn is the total number of Proffessors in the system (if everybody are friends),
    thus its doesnt depend on parameter t at ll.
     */
